 ...
 */
	...
	$('.blog-list__item').click(function () {
		...
		if ($(this).hasClass('open')) {
			$(this)
				.find('.description__title iconify-icon')
				.attr('icon', 'ic:round-minus');
		} else {
			$(this)
				.find('.description__title iconify-icon')
				.attr('icon', 'ph:plus-bold');
		}
	});
	...
	$('.s-know-products__nav .nav__item button').click(function () {
		...
		$('.s-know-products')
			...
			.addClass(settings.sectionClass);

		$('.s-know-products__content').hide();
		$('.' + settings.contentClass).show();

		if (buttonType === 'oi-e') {
			$('.s-know-products__nav .nav__item button')
				.removeClass('white')
				.addClass('black');
